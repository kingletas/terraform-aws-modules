...
```hcl
module "trail" {
  source = "github.com/kingletas/terraform-aws-modules//modules/cloudtrail-trail?ref=v0.1.0"
...
## Multi-region is the point

A single-region trail records nothing about the regions you are not watching, which is precisely where unwanted activity tends to appear — somebody's credentials mining in ap-south-1 shows up in no trail scoped to us-east-1.

`include_global_service_events` covers IAM, STS and CloudFront, which are global and otherwise recorded nowhere at all.

## Notes

- **`enable_log_file_validation` is what makes the trail evidence.** It writes digest files so a tampered or deleted log can be detected. Without it the trail is a convenience.
- The S3 bucket policy must already allow the CloudTrail service to write; this module does not edit another module's bucket.
...
- Delivering to CloudWatch Logs is what makes events queryable and alarmable. The S3 copy alone is storage.
...
| data\_events | Data event selectors keyed by a stable name, for object-level S3 or Lambda invocation logging. These are billed per event and a busy bucket generates a great many. | <pre>map(object({<br/>    resource_type             = string<br/>    resource_values           = list(string)<br/>    read_write_type           = optional(string, "All")<br/>    include_management_events = optional(bool, true)<br/>  }))</pre> | `{}` | no |
