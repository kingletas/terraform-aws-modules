...
A user pool with its app clients, threat protection on and user enumeration closed.

## Usage

```hcl
module "users" {
  source = "github.com/kingletas/terraform-aws-modules//modules/cognito-user-pool?ref=v0.1.0"
...
- **`prevent_user_existence_errors` is on.** Without it a failed login says whether the account exists, which turns the login form into a way to enumerate your users.
...
- `advanced_security_mode` is billed per monthly active user. `AUDIT` records risk without acting on it, which is the right place to start.
...
| advanced\_security\_mode | Threat protection: OFF, AUDIT to record risk, or ENFORCED to act on it. Billed per active user. | `string` | `"AUDIT"` | no |
