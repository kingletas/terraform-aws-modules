output "instances" {
  ...
  value = {
    for name, instance in aws_instance.this : name => {
      ...
      public_ip         = try(aws_eip.this[name].public_ip, instance.public_ip)
...
output "by_role" {
  ...
  value = {
    ...
    role => sort([for name, instance in local.instances : name if instance.role == role])
  }
}
...
output "private_ips_by_role" {
  ...
  value = {
    for role in distinct([for name, instance in local.instances : instance.role]) :
    role => [for name, instance in local.instances : aws_instance.this[name].private_ip if instance.role == role]
  }
}
...
output "instance_ids_by_role" {
  ...
  value = {
    for role in distinct([for name, instance in local.instances : instance.role]) :
    role => [for name, instance in local.instances : aws_instance.this[name].id if instance.role == role]
  }
}

output "elastic_ips" {
  description = "Elastic IPs, keyed by instance name. Empty for roles that did not ask for one."
  value       = { for name, address in aws_eip.this : name => address.public_ip }
}
