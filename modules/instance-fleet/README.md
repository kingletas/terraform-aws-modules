...
Where `ec2-instance` runs N identical instances, this runs a **fleet**: web nodes, a cron node, an admin node, a bastion, a builder — all from one AMI and one security posture, differing only where they genuinely differ.

## Usage

```hcl
module "fleet" {
  source = "github.com/kingletas/terraform-aws-modules//modules/instance-fleet?ref=v0.1.0"
...
A role names only what differs. The bastion above states four things; everything else — AMI, security groups, instance profile, encryption, IMDSv2 — comes from `defaults` and stays consistent across the fleet by construction.

The reference implementation this replaces built the same fleet by `merge()`-ing a base map four levels deep and carried its own comment reading *"this seriously needs some major refactoring"*. That is the problem being solved here.
...
Most real estates have both: an autoscaling web tier, and a handful of named nodes that do jobs exactly one machine may do.
