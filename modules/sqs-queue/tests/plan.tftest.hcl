...
run "attaches_no_policy_by_default" {
  ...
  assert {
    condition     = length(aws_sqs_queue_policy.this) == 0 && length(data.aws_iam_policy_document.this) == 0
    error_message = "With no services and no policy, the queue must have no resource policy."
  }
}
...
run "lets_services_publish_from_this_account" {
  ...
  variables {
    sending_services = ["cloudwatch.amazonaws.com", "backup.amazonaws.com"]
  }

  assert {
    condition     = length(aws_sqs_queue_policy.this) == 1
    error_message = "Naming sending services must attach a queue policy."
  }

  assert {
    condition     = one(data.aws_iam_policy_document.this[0].statement).actions == toset(["sqs:SendMessage"])
    error_message = "The service grant must allow sending and nothing else."
  }
  ...
  assert {
    ...
    error_message = "The service grant must name exactly the sending services."
  }
...
run "limits_services_to_source_arns" {
  ...
  variables {
    sending_services    = ["events.amazonaws.com"]
    sending_source_arns = ["arn:aws:events:us-east-1:123456789012:rule/plan-test"]
  }
...
run "merges_the_service_grant_into_the_caller_policy" {
  ...
  variables {
    sending_services = ["cloudwatch.amazonaws.com"]
    attach_policy    = true
    policy_json      = "{\"Version\":\"2012-10-17\",\"Statement\":[]}"
  }

  assert {
    condition     = length(aws_sqs_queue_policy.this) == 1 && tolist(data.aws_iam_policy_document.this[0].source_policy_documents) == tolist(["{\"Version\":\"2012-10-17\",\"Statement\":[]}"])
    error_message = "The caller's policy must be merged with the service grant into one queue policy."
  }
}
