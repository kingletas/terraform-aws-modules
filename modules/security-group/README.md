...
```hcl
module "app_sg" {
  source = "github.com/kingletas/terraform-aws-modules//modules/security-group?ref=v0.1.0"
...
Every rule needs exactly one source, and the module rejects a plan where that is not true:
...
Set `ip_protocol = "-1"` for all protocols; the port fields are then ignored.
...
- Every rule needs a non-empty description. This is checked at plan time.
...
| egress\_rules | Outbound rules keyed by a stable name. Leave empty to allow all outbound IPv4 traffic. | <pre>map(object({<br/>    description                  = string<br/>    from_port                    = optional(number)<br/>    to_port                      = optional(number)<br/>    ip_protocol                  = optional(string, "tcp")<br/>    cidr_ipv4                    = optional(string)<br/>    cidr_ipv6                    = optional(string)<br/>    referenced_security_group_id = optional(string)<br/>    prefix_list_id               = optional(string)<br/>    self                         = optional(bool, false)<br/>  }))</pre> | `{}` | no |
