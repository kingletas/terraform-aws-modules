...
```hcl
module "certificate" {
  source = "github.com/kingletas/terraform-aws-modules//modules/acm-certificate?ref=v0.1.0"
...
`validated_arn` only resolves once issuance completes, so anything depending on it waits. Point load balancers and CloudFront at that one.
...
- With no `zone_id`, the module creates no validation records and exports `validation_records` instead — give those to whoever runs the zone.
...
| wait\_for\_validation | Block the apply until the certificate is issued. Requires zone\_id, and can take several minutes. | `bool` | `true` | no |
...
| validated\_arn | ARN that only resolves once the certificate is issued, so a listener cannot attach to a pending one. |
