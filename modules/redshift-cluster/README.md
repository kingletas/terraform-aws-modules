...
```hcl
module "warehouse" {
  source = "github.com/kingletas/terraform-aws-modules//modules/redshift-cluster?ref=v0.1.0"
...
Turning it on requires that the cluster can actually reach S3 from inside the VPC — a gateway endpoint, or a NAT gateway. Without one, `COPY` stops working entirely, which is the failure people hit when they enable it on an existing cluster.
...
- **A single-node cluster is a different shape.** `number_of_nodes = 1` sets `cluster_type = "single-node"` and omits the node count, which the module handles — but a single node has no replication and no failover.
...
- `require_ssl = true` is in the default parameters. Clients that do not negotiate TLS stop connecting.
...
| parameters | Cluster parameters. A parameter group is created only when this is non-empty. | `map(string)` | <pre>{<br/>  "enable_user_activity_logging": "true",<br/>  "require_ssl": "true"<br/>}</pre> | no |
