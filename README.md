...
Fifty-two reusable Terraform modules for AWS, drawn from infrastructure I have actually run: networking, compute, data, storage, messaging, edge, identity and operations.

Each module does one thing and is composed by the caller. Every input is typed, described and validated where it can be; every collection is keyed by a name you choose rather than by list position. The `examples/` directory holds working compositions you can apply as they are.
...
| [`nlb`](modules/nlb) | TCP and TLS listeners for what an ALB cannot carry |
| **Data** | |
...
| [`step-function`](modules/step-function) | A state machine, for work one function should not orchestrate |
| **Data movement** | |
...
|---|---|
| [`magento`](examples/magento) | **Adobe Commerce, end to end**: CloudFront and WAF at the edge, an autoscaling web tier, the two nodes Magento cannot replicate, Aurora, Valkey, OpenSearch and EFS, and configuration discovered by tag rather than written down |
...
| [`data-pipeline`](examples/data-pipeline) | **Step Functions batch load**: raw and curated zones, a run ledger, and alarms that tell "nothing to do" from "did not run" |
...
**The examples are the point.** Each one is a working composition with a README that says what it costs, which decisions are deliberate, and what it does not do. They carry the operational detail the modules cannot: why Magento gets exactly one cron node, why an SQS visibility timeout is six times the function timeout, why a spoke VPC has no NAT gateway of its own.

## Using a module

Point `source` at this repository and pin a tag. Pinning matters — without `ref`, `terraform init` takes whatever is on the default branch that day.
...
**While this repository is private**, fetch it over SSH instead — `git::ssh://git@github.com/kingletas/terraform-aws-modules.git//modules/vpc?ref=v0.1.0` — or the shorter form fails on authentication. [From nothing to a planned stack](docs/from-nothing.md) walks through both.

Or copy a module into your own repository. They have no dependencies on each other and no dependency on anything else here.
...
Modules declare a permissive range so they compose with whatever you already run. The examples pin exactly and commit a lock file, which is the right split: a library states what it tolerates, a deployment states what it was tested against.

## Conventions

These hold across every module, so one module behaves the way the last one did.
...
- **No provider blocks inside a module.** Modules declare what they require; the caller configures it. A module with its own provider cannot be used twice in one configuration.
...
- **Secure by default, with a way out.** Encryption on, public IPs off, IMDSv2 required, logging on. Each is a variable, so you can choose otherwise deliberately.
...
`make check` runs five lanes: formatting, validation, tflint, checkov and a documentation freshness check. A lane whose tool is not installed is **reported and exits non-zero** — it never counts as a pass. That is deliberate: a green run that quietly skipped half its checks is worse than a red one.
...
The input and output tables in each README sit between `BEGIN_TF_DOCS` and `END_TF_DOCS` markers and are generated. Edit the variable, then run `make docs`.
...
The scan reports **385 passed and 0 failed**, and `.checkov.yml` carries a skip list to get there. That list is worth reading rather than trusting, so it is grouped by *why* each check cannot pass, with a comment on every entry:

- **The caller decides, and checkov cannot see across a module boundary.** Each of these is a variable on the module — access logging, enhanced monitoring, a customer-managed key. Checkov resolves the declaration, not the value a caller passes, so it reads every one as unset.
- **A deployment concern no single module can satisfy.** Whether a volume is in a backup plan, or a load balancer behind a WAF, is true of a stack rather than of a resource. Answering it would mean a module reaching into another module.
- **Checkov is wrong about the design.** It flags the ALB's plain HTTP listener for not using TLS 1.2, when that listener exists solely to answer port 80 with a redirect. It flags a KMS key policy for `resources = ["*"]`, which in a key policy is mandatory and means *this key*.
- **A real gap, named rather than hidden.** CloudFront origin failover is not implemented yet. It is in the skip list so the lane can be silent, and it is labelled as a gap so nobody mistakes it for a decision.

A check with no reachable silent case stops being a check and becomes something people learn to scroll past. **A repository that deploys these modules should run checkov with none of these skips** — that is where they can be answered.

## What has been verified

Every module and example is checked statically, and every module is planned against mock providers — through an example that composes it, or through a test of its own. Nothing has been applied.

| Check | What it proves |
|---|---|
| `terraform fmt -check -recursive` | formatting |
| `terraform validate` | every module and example parses and type-checks with its variables unknown |
| **`terraform test` with mock providers** | **every module plans with real values**, including IDs that do not exist until apply — see below |
...
`terraform validate` runs with every variable unknown, so it cannot see an expression that fails only once real values arrive. The plan tests evaluate every example with invented but realistic values, against [mock providers](testing/mocks) that stand in for AWS. They need no credentials and create nothing.

They earned their place on their first run. Validation passed every module, and planning found errors that would have broken a real first `terraform plan`:

- **Unknown `for_each` keys.** A collection keyed by values that do not exist until apply — a load balancer ARN, a subnet ID — cannot be planned. Every collection that becomes resources is now a map with static keys.
- **`count` on whether a computed value is null.** Terraform cannot tell an unknown string is not null until apply. Modules now decide whether a resource exists with a bool or an object, never a nullable string.
...
- **Values the provider rejects** — an invented security policy name and a log retention CloudWatch does not accept.

**A module you have not planned is unproven.** Run `make plan-test` after changing one.

### What cannot be tested locally

**About twenty modules cannot be exercised against a local AWS emulator at all**, because the emulator does not implement the service: RDS and Aurora, OpenSearch, ElastiCache, DocumentDB, Redshift, MWAA, DMS, Transfer Family, CloudFront, WAF, Cognito, Transit Gateway, Client VPN, Backup and CloudTrail. They are planned and statically checked, and that is all.
