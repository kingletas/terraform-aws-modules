...
By the end of this you will have planned a real AWS stack from these modules — on your own machine, with no AWS account, in about ten minutes. Then you will use one module in a project of your own.
...
A library of Terraform modules for AWS, and eleven examples that put them together into things you would actually run — a Magento store, a container platform, a data warehouse.
...
**Only Terraform is needed to follow this guide.** The other three are for `make check`, which you run before changing a module.
...
A newer 1.x works for using the modules. The check scripts are stricter: they refuse a different version of tflint or terraform-docs, because each version formats its output differently and the checks would report false failures.

## Step 2: clone it

The repository is private, so your GitHub account needs access to it. Clone over SSH:
...
**That is a real plan of 32 resources** — a VPC, subnets across two zones, a NAT gateway, flow logs, an IAM role and two instances — checked without touching AWS.
...
If one fails, the error names the module and the line. The most common cause is a value that only exists after apply being used as a `for_each` key; the fix is to key on a name you know in advance.
...
The plan shows what would be created and what it will cost you. **Read it before applying.** This example runs a NAT gateway, which is billed by the hour whether anything uses it or not.
...
And when you are finished, so the NAT gateway stops billing:

```bash
terraform destroy
```

**About twenty modules have never been applied against AWS by this project** — the ones a local emulator cannot run, like RDS, OpenSearch and CloudFront. They are planned and statically checked. Apply them somewhere you can afford a mistake first.
...
**The `git::ssh://` form matters while the repository is private.** The shorter `github.com/…` form in the module READMEs fetches over HTTPS, and Terraform will fail with an authentication error unless git has HTTPS credentials for GitHub.
...
Each module's README lists every input and output. Required inputs have no default; everything else has one chosen to be safe.
...
- **Plans do not depend on values from apply.** Every collection that becomes resources has keys you know in advance, so a first `terraform plan` works.
- **Each example says what it costs** and what it deliberately does not do.

## Where to go next

- [The module list](../README.md#what-is-here) — what each module builds
